    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `Recipe` (`id` INTEGER PRIMARY KEY AUTOINCREMENT, `title` TEXT NOT NULL, `howToMake` TEXT NOT NULL)');
...
class _$RecipeDao extends RecipeDao {
  ...
  Future<List<Recipe>> getAllRecipe() async {
    return _queryAdapter.queryList('SELECT * FROM Recipe',
        mapper: (Map<String, Object?> row) => Recipe(
            ...
            howToMake: row['howToMake'] as String));
  }
  ...
  @override
  Future<List<Component>?> getAllComponentByRecipeTitle(String title) async {
    await _queryAdapter.queryNoReturn(
        'SELECT Component.name FROM Component, Recipe WHERE belongTo = ?1',
        arguments: [title]);
  }
